# Baseline: w/o context information

mask=True
attr="CEL-C10D5-nodesc"
...
for dict_irrelevant_rate_curriculum in "on"
do

for dictionary_mask_rate_curriculum in "on"
do
...
CoT_on=True
job_name=$attr"_"$n_sample"_DR"$icl_im_datamix"_"$dict_irrelevant_rate_curriculum"_"$dictionary_mask_rate_curriculum"_"$dictionary_mask_token_direction"_"$dictionary_mask_config"_"$CoT_on"_"$weight_decay"_"$additional_args

echo $job_name

# bash scripts/train_im_ICLDR.sh \
sbatch -J $job_name scripts/train_CEL.sh \
    $attr \
    $train_ds_IM \
    $test_ds_IM \
    $save_freq \
