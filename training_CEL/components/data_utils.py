...
def fake_dictionary(dictionary, parsed_source, irrelevant_rate=0):
    """
    Filter the dictionary based on the parsed source and irrelevant rate
    """
    assert irrelevant_rate >= 0 and irrelevant_rate <= 1, "Invalid irrelevant rate, should be in [0, 1]"
    ...
    value_ls = list(dictionary.values())
    key_map = {key_ls[i]: key_ls[perm1[i]] for i in range(N)}
    value_map = {value_ls[i]: value_ls[perm2[i]] for i in range(N)}

    for k in total_keys:
        fake_dictionary[key_map[k]] = value_map[dictionary[k]]
...
def encode_dictionaries(
    ...
    for i, dictionary in enumerate(dictionaries):
        ...
        if data_args.fake_dictionary:
            filtered_dict = fake_dictionary(dictionary, parsed_sources, dictionary_filter_rates[i])
        else:
...
class OnTheFlyDataset(Dataset):
    ...
    def get_sample(self, idx):
        ...
        if self.training_type == 'IM' and self.n_textbook > 1:
...
class MergeDataset(Dataset):
    def __init__(self, datasets):
        ...
        self.size = sum([len(ds) for ds in datasets])
        self.sample_mapping = self.get_item_split()

        for dataset in self.datasets:
            dataset.override_seed = True
        print(f"Merging {len(self.datasets)} datasets", flush=True)

    def get_item_split(self):
        ret = []
        sizes = np.array([len(ds) for ds in self.datasets])
        enum_tracker = np.zeros(len(self.datasets))

        for _ in range(self.size):
            remaining_counter = sizes - enum_tracker
            if np.sum(remaining_counter) == 0:
                raise IndexError("Index out of bounds")
            remaining_prob = remaining_counter / remaining_counter.sum()
            next_ind = np.random.choice(len(self.datasets), p=remaining_prob)

            ret.append([next_ind, int(enum_tracker[next_ind])])
            enum_tracker[next_ind] += 1
        return ret
    ...
    def get_item(self, idx):
        np.random.seed(idx)
        split, ind = self.sample_mapping[idx]
        # print(idx, split, ind)
        # print(self.datasets[split][ind])
        return self.datasets[split][ind]

def encode_dataset_IM(
        tokenizer,
        ds_type,
        data_args,
        curriculum_args,
    ...
    if ds_type == 'train':
        ds_path = data_args.train_path_IM
        n_textbook = data_args.train_n_textbook_IM
        n_sample = data_args.train_n_sample_IM
        ds_source = 'train'

    elif 'train' in ds_type: # generating in-domain eval dataset
        ds_path = data_args.train_path_IM
        n_textbook = data_args.eval_n_textbook_IM
        n_sample = data_args.eval_n_sample_IM
        ds_source = 'train'
    
    elif 'test' in ds_type:
        ds_path = data_args.eval_path_IM
        n_textbook = data_args.eval_n_textbook_IM
        n_sample = data_args.eval_n_sample_IM
        ds_source = 'test'
    ...
    eval_n_textbook = data_args.eval_n_textbook_IM
    eval_n_sample = data_args.eval_n_sample_IM

    assert n_sample % n_textbook == 0
    assert eval_n_sample % eval_n_textbook == 0
    
    print(f"Loading IM dataset {ds_type} with {n_textbook} textbooks", flush=True)
    ...
    for i in range(len(dataset)):
        ...
        # print (len(dataset[i][1]))
        # print (len(dataset[i]))
    
    print(f"Encoding IM dataset {ds_type} \
...
def encode_dataset_ICL(
    ...
    if ds_type == 'train':
        ds_path = data_args.train_path_ICL
        n_textbook = data_args.train_n_textbook_ICL
        n_sample = data_args.train_n_sample_ICL
        ds_source = 'train'

    elif 'train' in ds_type: # generating in-domain eval dataset
        ds_path = data_args.train_path_ICL
        n_textbook = data_args.eval_n_textbook_ICL
        n_sample = data_args.eval_n_sample_ICL
        ds_source = 'train'
    
    elif 'test' in ds_type:
        ds_path = data_args.eval_path_ICL
        n_textbook = data_args.eval_n_textbook_ICL
        n_sample = data_args.eval_n_sample_ICL
        ds_source = 'test'
    ...
    eval_n_textbook = data_args.eval_n_textbook_ICL
    eval_n_sample = data_args.eval_n_sample_ICL

    assert n_sample % n_textbook == 0
    assert eval_n_sample % eval_n_textbook == 0
    
    print(f"Loading ICL dataset {ds_type} with {n_textbook} textbooks", flush=True)
    ...
    print(f"Encoding ICL dataset {ds_type} \
            ({len(dataset)} tutorial, {n_sample} samples),\n \
            CoT on: {data_args.cot_on},\n \
            dict_ip: {dict_ip},\n \
            think_rate: {think_rate},\n \
