...
n_batches = 500
print(f'batchsize: {batchsize}', flush=True)

mask_sizes = list(range(1, N**2, N//2))

stds = []
accs = []
means = []

for num_mask_inds in tqdm(mask_sizes):
    ...
    for i in range(n_batches):
        ...
        mask_inds = [list(np.random.choice(N**2, num_mask_inds, replace=False))] + [[]] * (D - 1)
...
os.makedirs(f'./logs/var_dropout/N{N}D{D}L{L}T{temp}', exist_ok=True)
json.dump(meta, open(f'./logs/var_dropout/N{N}D{D}L{L}T{temp}/btsize{batchsize}.json', 'w'))